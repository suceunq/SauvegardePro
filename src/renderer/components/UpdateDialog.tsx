  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70" onClick={fermer}>
    <div className="w-[460px] rounded-2xl border border-slate-700 bg-slate-900 shadow-2xl" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="space-y-4 p-5 text-sm">
        ...
        {etat.phase === 'telechargement' && <div><p className="mb-2">Téléchargement en cours…</p><div className="h-2 overflow-hidden rounded bg-slate-700"><div className="h-full bg-blue-500" style={{ width: `${etat.progressionPourcent ?? 0}%` }} /></div><p className="mt-1 text-xs text-slate-400">{etat.progressionPourcent ?? 0}% · {etat.octetsParSeconde ? `${(etat.octetsParSeconde / 1048576).toFixed(1)} Mo/s` : 'calcul…'} · Temps restant estimé : {etat.secondesRestantes ?? '…'} s</p></div>}
